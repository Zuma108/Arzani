 ...
 * Legal Agent Routes
 * 
 * Route handlers for the Legal Specialist Agent's A2A endpoints
 */
...
import { processLegalTask } from './legal.js';

/**
 * Handle tasks/send requests for the Legal Agent
 * 
...
export async function handleTaskSend(req, res) {
  ...
  try {
    ...
    const workingTask = {
      ...requestTask,
      state: 'working',
      agentId: 'legal-agent'  // Legal agent identifier
    };
    
    // Process the legal task based on the user's message
    const { task: responseTask, message: responseMessage } = await processLegalTask(workingTask, requestMessage);
    ...
    console.error(`Error processing legal task: ${err.message}`);
...
/**
 * Register legal agent routes with Express app
 * 
...
export default registerLegalRoutes;