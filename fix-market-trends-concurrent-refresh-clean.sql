...
-- Step 3: Test the existing setup and refresh the view
...
-- Step 5: Create an RDS-optimized refresh function with robust error handling
...
-- Step 6: Recreate the trigger with RDS-optimized settings
CREATE TRIGGER refresh_market_trends
    AFTER INSERT OR UPDATE OR DELETE ON businesses
    FOR EACH STATEMENT
    EXECUTE FUNCTION refresh_market_trends_mv();

-- Step 7: Test the setup with a manual refresh
DO $test$
BEGIN
    BEGIN
        REFRESH MATERIALIZED VIEW market_trends_mv;
...
-- Step 8: Verify the setup and provide status report
DO $verify$
DECLARE
    row_count integer;
