...
/**
 * Helper function to extract and clean blog content from full HTML documents
 */
function extractBlogContent(htmlContent) {
  if (!htmlContent) return '<p>Content not available.</p>';
  
  // If content is already clean (no DOCTYPE), return as is
  ...
  if (bodyStart !== -1 && bodyEnd !== -1) {
    let content = htmlContent.substring(bodyStart + 6, bodyEnd);
    
    // Remove any remaining style tags
    content = content.replace(/<style[^>]*>[\s\S]*?<\/style>/gi, '');
    
    // Remove the title h1 if it exists (we already have it in the header)
  ...
  // Fallback: try to remove DOCTYPE, html, head, and body tags
...
async function getBlogPostByPath(req, res) {
  try {
    ...
    const cleanedPost = {
      ...post,
      content: extractBlogContent(post.content)
    };
