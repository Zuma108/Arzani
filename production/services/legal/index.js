 ...
 * Legal Agent Server
 * 
 * Implements the Legal Specialist Agent for the Arzani A2A system
 * - Provides legal document generation (NDAs, LOIs)
 * - Offers compliance guidance
 * - Handles regulatory questions for business transactions
 */
...
import { registerLegalRoutes } from './routes.js';

// Load environment variables
dotenv.config();

const app = express();
const PORT = process.env.LEGAL_AGENT_PORT || 5003;
...
app.use((req, res, next) => {
  console.log(`[${new Date().toISOString()}] ${req.method} ${req.path} (legal-agent)`);
  next();
});
...
// Start the server
app.listen(PORT, () => {
  console.log(`Legal Agent running on port ${PORT}`);
